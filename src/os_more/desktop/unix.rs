...
pub fn get_desktop_items(query_user: Option<String>, _attr_filter: Option<u32>, filters: &Vec<String>) -> Vec<DesktopItem> {
  let mut items = vec![];
...
fn process_desktop_entries(desktop_path: &PathBuf, uname: &str, filters: &Vec<String>, items: &mut Vec<DesktopItem>) {
  if let Ok(entries) = std::fs::read_dir(desktop_path) {
    for entry in entries.flatten() {
      ...
      let attribute = entry.metadata().map(|meta| meta.mode()).unwrap_or(0);
