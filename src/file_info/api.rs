use super::{ArchType, Dependency, ExeType, ImportedFunction, PlatformType};
...
pub async fn file_info_query<T: AsRef<str>>(task: &str, args: impl IntoIterator<Item = T>) -> e_utils::AnyResult<String> {
  ...
  match &*task {
    "copy-lib" => {
      ...
      to.auto_create_dir()?;
      Ok(lib_copy(src, to).map(|v| format!("Copy count {}", v))?)
    }
    "print" => {
  ...
  match Object::parse(&buffer)? {
    Object::Elf(elf) => {
      ...
      if is_android {
        ...
        slf.exe_type = ExeType::from_android(elf.header.e_type, elf.is_lib);
      } else {
        slf.platform = PlatformType::Linux;
        slf.exe_type = ExeType::from_linux(elf.header.e_type, elf.is_lib);
      }
      slf.is_64 = elf.is_64;
    ...
    Object::PE(pe) => {
      ...
      slf.exe_type = if pe.is_lib { ExeType::Dll } else { ExeType::Exe };
      slf.is_64 = pe.is_64;
      ...
      if let Some(first_member) = archive.members().get(0) {
        if let Ok(inner_object) = Object::parse(first_member.as_bytes()) {
          match inner_object {
            Object::Elf(_) => {
              slf.platform = PlatformType::Linux;
              slf.exe_type = ExeType::LinuxSharedObject;
            }
