...
pub async fn run_mode(mode: &dyn TestMode, ctx: &ModeContext<'_>, task: &str) -> e_utils::AnyResult<String> {
  ...
  // check 且 v3>0 时启动负载
  #[cfg(feature = "system")]
  let load_handles = if task == "check" && params.v3 > 0.0 {
    inst.spawn_load(ctx, params.v3)?
  } else {
    vec![]
  };
  ...
  let result: e_utils::AnyResult<String> = async {
    ...
    for i in 0..params.test_secs {
      ...
      for m in &last_metrics {
        let st = stats.entry(m.name.clone()).or_insert_with(|| MetricStat::new(&m.name, m.unit.clone(), m.value));
