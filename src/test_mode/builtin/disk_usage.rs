...
impl TestMode for DiskUsageMode {
  ...
  fn create(&self) -> e_utils::AnyResult<Box<dyn ModeInstance>> {
    Ok(Box::new(DiskUsage {
      disks: sysinfo::Disks::new_with_refreshed_list_specifics(sysinfo::DiskRefreshKind::nothing().with_storage().with_io_usage()),
    }))
  }
}
...
impl ModeInstance for DiskUsage {
  fn sample(&mut self, ctx: &ModeContext) -> e_utils::AnyResult<Vec<Metric>> {
    self
      .disks
      .refresh_specifics(false, sysinfo::DiskRefreshKind::nothing().with_storage().with_io_usage());
